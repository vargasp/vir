...
det_lets_list = [1,5,10,15,20]

s = np.empty([len(det_lets_list),63,64])

for i, det_lets in enumerate(det_lets_list):
    d = vir.Detector2d(nDets=nDets,dDet=dDet,offset=0.25,det_lets=det_lets)
    s[i,:,:] = pj.createParSinoClass(g,d,s,pd).squeeze() # [distance * linear attenuation]
