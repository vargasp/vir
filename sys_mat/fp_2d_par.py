...
def siddons_fp_2d(img, angles, nDets, dPix=1.0, dDet=1.0):
    nX, nY = img.shape
    sino = np.zeros((angles.size, nDets), dtype=np.float32)

    # Image edges (centered)
    x_edges = dPix * (np.arange(nX+1) - nX/2.0)
    y_edges = dPix * (np.arange(nY+1) - nY/2.0)
    ...
    for idxAngle, angle in enumerate(angles):
        ...
        for k in range(nDets):
            ...
            cross_list = []
            if dx != 0:
                cross_list += list((x_edges - x0) / dx)
            if dy != 0:
                cross_list += list((y_edges - y0) / dy)
            ...
            for seg in range(crosses.size-1):
                ...
                ix = int(np.floor((xc - x_edges[0]) / dPix))
                iy = int(np.floor((yc - y_edges[0]) / dPix))
                if 0<=ix<nX and 0<=iy<nY:
                    sino[idxAngle,k] += img[ix,iy] * abs(tc1-tc0)
    return sino


def joseph_fp_2d(img, angles, nDets, dPix=1.0, dDet=1.0):
    """
...
def separable_footprint_fp_2d(img, angles, nDets, dPix=1.0, dDet=1.0):
    """
...
img = np.zeros((32, 32))
img[16:24, 16:24] = 1.0  # center impulse
